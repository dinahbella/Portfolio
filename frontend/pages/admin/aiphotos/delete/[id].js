...
export default function DeletePhoto() {
  ...
  useEffect(() => {
    ...
    const fetchPhoto = async () => {
      try {
        const { data } = await axios.get(`/api/aiphoto?id=${id}`);
        setPhotoInfo(data);
      } catch (error) {
  ...
  const deletePhoto = async () => {
    try {
      await axios.delete(`/api/aiphoto?id=${id}`);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold ">
                  Delete <span className="text-red-600">{PhotoInfo.title}</span>
                </h1>
              </div>
              <div className="flex items-center text-sm ">
                <BsPostcard className="mr-2" />
                <span>/</span>
                <span className="ml-2">Delete Photo</span>
              </div>
            </div>
          </div>
        </header>
